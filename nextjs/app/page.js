...
export default function Home() {
  ...
  return (
    <SiteShell whatsappUrl={whatsapp()}>
      ...
      <div id="top" />
      <section className="hero"><div className="hero-media" /><div className="container hero-content reveal"><p className="eyebrow">حلول زراعية متكاملة للمشاريع والحدائق</p><h1>أفضل قيمة مقابل السعر في توريد الشتلات بالمملكة</h1><p className="hero-copy">نوفر مجموعة متنوعة من الشتلات والنباتات والخدمات الزراعية بأسعار تنافسية، مع حلول تناسب الفلل والحدائق والمزارع والمشاريع السكنية والتجارية.</p><div className="hero-actions"><a className="button button-gold" href={whatsapp()} target="_blank" rel="noopener noreferrer">اطلب عرض السعر عبر واتساب <span aria-hidden="true">↗</span></a><a className="text-link" href="#services">تعرّف على خدماتنا <span aria-hidden="true">←</span></a></div><p className="trust-line"><span aria-hidden="true">✦</span> أرسل متطلبات مشروعك، وسنساعدك في اختيار الحل المناسب.</p></div><div className="hero-scroll">مرر للاستكشاف <span aria-hidden="true">↓</span></div></section>

      <section className="section services" id="services"><div className="container"><Heading eyebrow="ما نقدمه لك" title={<>كل ما يحتاجه مشروعك<br/><em>الزراعي في مكان واحد</em></>} text="نوفر مجموعة متكاملة من المنتجات والخدمات الزراعية لتلبية احتياجات الأفراد والمشاريع بمختلف أحجامها."/><div className="services-grid">{services.map(([number,title,text,icon])=><article className="service-card reveal" key={number}><div className="card-top"><span className="service-number">{number}</span><span className="service-icon" aria-hidden="true"><LineIcon path={icon}/></span></div><h3>{title}</h3><p>{text}</p><a href={whatsapp()} target="_blank" rel="noopener noreferrer">اطلب التفاصيل <span aria-hidden="true">←</span></a></article>)}</div><div className="section-cta"><a className="button button-outline" href={whatsapp()} target="_blank" rel="noopener noreferrer">تحدث مع مستشار زراعي <span aria-hidden="true">↗</span></a></div></div></section>

      <section className="value-section" id="about"><div className="container value-grid"><div className="value-art reveal"><div className="image-frame"/><span className="stamp">من<br/>الأرض<br/>نبدأ</span></div><div className="value-copy reveal"><p className="eyebrow eyebrow-light">حلول مدروسة لمختلف الاحتياجات</p><h2>قيمة أفضل<br/><em>لمشروعك</em></h2><p>نحرص في حقول السلام على تقديم حلول زراعية تحقق أفضل قيمة ممكنة، من خلال توفير المنتجات والخدمات المناسبة بأسعار تنافسية تتوافق مع احتياجات المشروع.</p><a className="button button-gold" href={whatsapp()} target="_blank" rel="noopener noreferrer">احصل على عرض سعر مخصص <span aria-hidden="true">↗</span></a></div></div></section>
